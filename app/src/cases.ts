...
/** loyalty program: the legacy tiers, what the audit found, what we proposed */
export type CaseTier = { name: string; threshold: string; cashback: string };
export type CaseLoyalty = {
  title: string;
  lead: string;
  /** the program as it existed when we came in */
  tiers: CaseTier[];
  tiersNote: string;
  /** audit findings */
  found: string[];
  /** the redesigned concept */
  concept: { name: string; lead: string; points: string[] };
  /** modelled outcome, kept qualitative — absolute figures stay internal */
  effect: string;
};
...
export const caseStories: Record<string, CaseStory> = {
  ...
  vsyogazin: {
    ...
    task:
      'Запустить с нуля новое розничное и онлайн-направление внутри большого российского холдинга. Не «сделать сайт», а собрать бизнес целиком: продуктовую линейку, интернет-магазин с доставкой по всей стране, розничные точки, маркетинг, коммуникации и команду. Мы вошли в проект как часть команды заказчика: Максим — продукт-оунер на стадии разработки и затем e-com-директор, Нелли — руководитель PR и SMM.',
    build: [
      {
        t: 'Продукт: сайт и мобильное приложение',
        d: 'Роль продукт-оунера на стадии разработки: требования, структура каталога, клиентский путь, приёмка релизов и план запуска из 70+ задач. На выходе — интернет-магазин и мобильное приложение с оплатой и доставкой по всей России, а не витрина-визитка.',
      },
      {
      ...
      },
      {
        ...
        d: 'Участвовали в создании и внедрении единой программы для розницы и онлайна: правила начисления и списания, сгорание, возвраты, виртуальная карта и сценарии регистрации. Через год провели аудит — нашли дыры в экономике, аналитике и юридической части — и собрали концепцию следующей версии с финмоделью.',
      },
      {
        t: 'SMM-отдел и контент инхаус',
        d: 'Собрали SMM-направление с нуля и внутри компании: команда, процессы, контент-план и продакшн. Поддержка покупателей в соцсетях стала частью сервиса, а не отдельной подрядной услугой.',
      },
      {
    ...
    results: [
      ...
      { v: 'ROI >100%', l: 'окупаемость рекламных вложений' },
    ],
    stack: [
      {
        layer: 'Витрины',
    ...
    loyalty: {
      title: 'Лояльность: аудит и редизайн',
      lead: 'Мы участвовали во внедрении единой омниканальной программы для розницы и интернет-магазина — баллы с кассы тратятся онлайн и наоборот, процессинг на Loymax, виртуальная карта в приложении и Apple Wallet. А через год провели аудит того, что получилось, и собрали концепцию следующей версии.',
      tiers: [
      ...
      tiersNote: 'Действующая программа: четыре уровня по сумме накоплений, 1 балл = 1 рубль, оплата до 50% чека.',
      found: [
        'Названия уровней — «Серебро, Золото, Платина» — читались старомодно и не отражали бренд и его амбассадоров.',
        'Уровни назначались без опоры на историю покупок, а условия не регулировали переток клиентов на верхний уровень.',
        'Начисление баллов через 15 минут после покупки создавало риск фрода.',
        'Приветственные баллы и бонус на день рождения со сроком жизни в год не сходились по экономике.',
        'Отчётность в BI собиралась только по офлайн-чекам и расходилась с данными 1С — считать эффект было не на чем.',
        'Триггерные цепочки существовали в усечённом виде, а бумажная анкета нарушала требования юристов.',
      ],
      concept: {
        name: '«Всёдрузья»',
        lead: 'Вместо статусов по размеру кошелька — круг друзей бренда. Концепция выросла из девиза «Всё для всех» и из характеров амбассадоров: щедрые быстрые механики от Ляйсан и выгодные «умные» — от Павла.',
        points: [
          'Отказ от уровней: привилегии одинаковые для всех участников',
          'Всёбаллы: 250 приветственных, 300 в день рождения, 150 за друга',
          'Базовый кэшбэк 5% и ×2 за рекомендованные товары',
          'Короткие «горячие» баллы на 14 дней и базовые — на год',
          'Оплата до 50% покупки, единый баланс в рознице и онлайне',
          'Реактивация оттока и предоттока как отдельная механика',
        ],
      },
      effect: 'Финмодель на период с октября 2023 по декабрь 2024 показала кратный прирост дополнительной валовой прибыли относительно действующей программы — в основном за счёт продвижения высокомаржинальных товаров удвоенным кэшбэком и возврата клиентов из оттока.',
    },
    gallery: [
    ...
    highlights: [
      ...
      },
      {
        ...
        d: 'Отдел построили с нуля внутри компании — от найма и процессов до контента и поддержки покупателей. Экспертиза осталась в команде, а не ушла вместе с подрядчиком.',
      },
      {
        t: 'Офлайн и онлайн в одной механике',
        d: 'Чековое промо связало розницу и интернет-магазин: единый розыгрыш с призами вплоть до смартфона и поездки на отдых — и первые данные о покупателях из обоих каналов.',
      },
      {
        t: 'Собственная марка, а не перепродажа',
    ...
    quote: {
      text: 'Мы пришли не «сделать сайт». Полтора года всей командой строили бизнес: продукт, магазин, доставку, розницу и людей — и довели его до реальных продаж.',
      author: 'Команда МЭПЛ',
    },
    epilogue: {
      t: 'Что стало с проектом',
      d: 'В 2024 году магазин активно наращивал аудиторию и уже начал приносить деньги, но проект был закрыт: на фоне изменившейся экономической ситуации холдинг сократил финансирование новых направлений. Интернет-магазин и розничные точки сегодня не работают. Для нас это по-прежнему один из самых сильных кейсов: за полтора года мы прошли весь путь от идеи до работающего бизнеса и собранной команды, а решение о закрытии лежало за пределами продукта и маркетинга.',
    },
